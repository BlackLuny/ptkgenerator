...
use crate::server_consumer::client_proc::ClientShareInfo;
...
pub async fn async_decode(
    share_info: Arc<ClientShareInfo>,
    data: &mut [u8],
    cacher: &mut MemCacher,
    data_rst: &mut HashMap<usize, [InsInfo; 4096]>,
) -> usize {
    ...
    let f = move |rst: &mut [u8], addr: u64| {
        ...
        //     return 0;
        // }
        println!("read memory {:?}", addr);
        share_info.read_mem(addr as usize, rst.len(), rst);
        rst.len()
    };
    cacher.f = Some(Box::new(f));
