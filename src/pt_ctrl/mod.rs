...
struct PtSetupInfo {
    ...
    event_num: u32,
    no_pmi: u32,
    event_info: [EnventInfo; MAX_CPU_NUM],
    addrs_cfg: [AddrRange; 4],
}
...
pub fn read_process_memory_drv(
    h: HANDLE,
    pid: u32,
    address: u64,
    len: u16,
    ...
    let mut read_req = PtCopyMemReq {
        pid: pid as u64,
        target_ptr: address,
        localbuf: ouput_buff.as_mut_ptr() as u64,
        size: len as u64,
        write: 0,
    };
...
pub fn write_process_memory_drv(
    h: HANDLE,
    pid: u32,
    address: u64,
    buff: &Vec<u8>,
...
fn setup_pt_no_pmi_start(
    ...
    let mut setup_info = PtSetupInfo {
        ...
        addrs_cfg: [
            AddrRange {
                cfg_mode: addr_cfg as u64,
                addr_n_a: addr_start as u64,
                addr_n_b: addr_end as u64,
            },
            AddrRange::new(),
            AddrRange::new(),
            AddrRange::new(),
    ...
    let r = unsafe {
        DeviceIoControl(
            ...
            setup_info_ptr,
            stru_size as DWORD,
            rsp as *mut _ as LPVOID,
            rsp_size as DWORD,
            (&mut rst_len) as *mut _ as LPDWORD,
            null_mut(),
        )
    };
...
fn fetch_data(buff: *mut u8, buff_len: usize, pos: usize, out_buff: &mut Vec<u8>) -> usize {
    ...
    for i in 0..out_buff.len() {
        ...
        out_buff[i] = unsafe { *(buff.add(offset)) };
        unsafe {
...
where
    F: FnMut(usize, &Vec<u8>, usize) -> bool,
{
    let mut rsp = Default::default();
    setup_pt_no_pmi_start(
        h, pid, buff_size, mtc_freq, psb_freq, cyc_thld, addr_cfg, addr_start, addr_end, &mut rsp,
    )
    .expect("Setup pt faile");
    // start capture data
...
pub fn get_pt_handle(pt_device: &str) -> Result<HANDLE, DWORD> {
    let len = unsafe {
        MultiByteToWideChar(
            CP_ACP,
            0 as DWORD,
            pt_device.as_ptr() as LPCSTR,
            pt_device.len() as i32,
            null_mut(),
            0,
        )
    };
