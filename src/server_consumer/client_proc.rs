...
use crate::{decode_proc::{async_decode, decode}, mem_cacher::MemCacher};

use super::{client_pool::ClientPool, communator::{Communator, CommunatorSender}, interface::PtProcMsg, msg_adapter::PtProcMsgAdapter};

pub struct ClientShareInfo {
    send_mem_req: Option<crossbeam::channel::Sender<(usize, usize, crossbeam::channel::Sender<Vec<u8>>)>>,
}
impl ClientShareInfo {
    fn new(send_mem_req_tx: crossbeam::channel::Sender<(usize, usize, crossbeam::channel::Sender<Vec<u8>>)>)->Self {
        ClientShareInfo {send_mem_req:Some(send_mem_req_tx)}
    }
    pub fn read_mem(&self, addr: usize, len: usize, output_buff: &mut [u8]) {
        let (tx, mut rx) = crossbeam::channel::bounded::<Vec<u8>>(1);
        self.send_mem_req.as_ref().unwrap().send((addr, len, tx)).unwrap();
...
async fn proc_data(share_data: Arc<ClientShareInfo>, proc_rx: crossbeam::channel::Receiver<ProcCtrl>, sender: Arc<CommunatorSender<PtProcMsgAdapter>>)
{
    ...
    while let Ok(ctrl) = proc_rx.recv() {
        match ctrl {
            ...
            ProcCtrl::Proc(mut data) => {
                ...
                async_decode(share_data.clone(), &mut data, &mut cacher, &mut data_rst_tmp).await;
                // 转换
...
impl ServerAgent {
    pub async fn connect(address: &str) ->Self {
        let (mem_req_tx, mem_req_rx) = crossbeam::channel::unbounded::<(usize,usize, crossbeam::channel::Sender<Vec<u8>>)>();
        ...
        let h1 = tokio::spawn (async move {
            ...
            let h2 = tokio::spawn (async move {
                while let Ok((addr, len, rsp_ch)) = mem_req_rx.recv() {
                    ...
                    if sender_clone.send_data(PtProcMsg::MemReq(addr, len)).await.is_err() {
                        break;
                    }
            ...
            for i in 0..2 {
                tokio::spawn( proc_data(share_data.clone(), proc_rx.clone(), sender.clone()));
            }
            println!("start listenning");
            while let Ok(msg) = receiver.recv_data().await {  // 处理recv请求时不能阻塞，否则永远无法接收服务器的消息
                match msg {
                    PtProcMsg::Init() =>{
                        println!("recv init");
